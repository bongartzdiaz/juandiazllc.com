...
async function main() {
  ...
  const dealSeed = [
    ...
    // Bundle DI — extra demo rows so a sales walkthrough sees a
    // realistic spread of stages + outcomes, not a 2-card empty-ish
    // pipeline. Re-runs of `npm run seed` are idempotent (upsert by id).
    {
      id: 'seed-deal-waterworks',
      title: 'WaterWorks — Drenthe pilot rollout',
      pipelineId: pipeline.id,
      stageId: 'seed-stage-new',
      contactId: 'seed-contact-marcus@waterworks.io',
      ownerId: admin.id,
      valueCents: 1_800_000,
      probability: 25,
      status: 'open',
      dealType: 'grant',
      expectedClose: new Date(Date.now() + 120 * 86400_000),
      notes: 'Inbound from sectors/energy. First call scheduled.',
    },
    {
      id: 'seed-deal-community',
      title: 'Community Network multi-year framework',
      pipelineId: pipeline.id,
      stageId: 'seed-stage-negotiation',
      contactId: 'seed-contact-elena@community.org',
      ownerId: admin.id,
      valueCents: 18_500_000,
      probability: 75,
      status: 'open',
      dealType: 'partnership',
      expectedClose: new Date(Date.now() + 30 * 86400_000),
      notes: 'Final redlines on legal — close target end of quarter.',
    },
    {
      id: 'seed-deal-solarcoop-renewal',
      title: 'Solar Co-op — 2026 renewal',
      pipelineId: pipeline.id,
      stageId: 'seed-stage-closed-won',
      contactId: 'seed-contact-david@solarcoop.com',
      ownerId: admin.id,
      ...
      notes: 'Renewed for another 12 months at slight uplift.',
    },
    {
      id: 'seed-deal-bensaid',
      title: 'Bensaid Capital pilot',
      pipelineId: pipeline.id,
      stageId: 'seed-stage-closed-lost',
      contactId: null,
      ownerId: admin.id,
      valueCents: 2_200_000,
      probability: 0,
      status: 'lost',
      dealType: 'partnership',
      actualClose: new Date(),
      notes: 'Lost to incumbent vendor — pricing mismatch on volume tier.',
    },
    {
      id: 'seed-deal-rotterdam',
      title: 'Rotterdam housing association — discovery',
      pipelineId: pipeline.id,
      stageId: 'seed-stage-new',
      contactId: null,
      ownerId: admin.id,
      ...
      status: 'open',
      dealType: 'partnership',
      expectedClose: new Date(Date.now() + 180 * 86400_000),
      notes: 'Inbound web form. Has not committed to a pilot yet.',
    },
  ]
