...
const RAW_PAYLOADS_DIR = path.join(DATA_DIR, 'raw-payloads')
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Save raw payload for debugging (keep last 100)
    await fs.mkdir(RAW_PAYLOADS_DIR, { recursive: true }).catch(() => {})
    const payloadFile = path.join(RAW_PAYLOADS_DIR, `${Date.now()}.json`)
    fs.writeFile(payloadFile, JSON.stringify(body, null, 2)).catch(() => {})
    // Cleanup old payloads (fire and forget)
    cleanupOldPayloads().catch(() => {})

    // Extract phone from multiple possible locations in payload
    const phone = body.contactPhone || body.contact?.phone || body.contact?.phoneNumber || body.phone || body.phoneNumber || ''
    const email = body.contactEmail || body.contact?.email || body.email || ''
    const contactId = body.contactId || body.contact?.id || body.id || ''

    // DM Champ webhook payload — extract relevant fields
    const event: DmChampEvent = {
      id: contactId || `evt_${Date.now()}`,
      event: body.event || body.type || 'unknown',
      contactName: body.contactName || body.contact?.name || body.contact?.firstName || body.name || 'Onbekend',
      contactPhone: phone,
    ...
    // Proactive enrichment: try phone first, then email
    if (phone && phone.length > 5) {
      enrichContactInBackground(phone).catch(() => {})
    } else if (email && email.includes('@')) {
      // Try lookup by email if no phone available
      enrichContactByEmail(email).catch(() => {})
    }

    console.log(`[Webhook] DM Champ event: ${event.event} — ${event.contactName} (phone: ${phone || 'none'}, email: ${email || 'none'})`)
...
// Enrich contact by email when phone is not available
async function enrichContactByEmail(email: string) {
  const apiKey = process.env.DMCHAMP_API_KEY
  ...
  try {
    const res = await fetch(`${baseUrl}/contacts?apiKey=${apiKey}&email=${encodeURIComponent(email)}`)
    if (!res.ok) return
    const json = await res.json()
    if (!json.success || !json.contact) return
    ...
    if (!phone || phone.length <= 5) return // Need phone for cache key
    ...
    await fs.writeFile(API_CACHE_FILE, JSON.stringify(cache, null, 2))
    console.log(`[Webhook] Enriched by email: ${apiContact.firstName} ${apiContact.lastName} (${email} → ${phone})`)
  } catch (err) {
    console.error(`[Webhook] Email enrich failed for ${email}:`, err instanceof Error ? err.message : err)
  }
}

// Cleanup raw payloads older than 100 files
async function cleanupOldPayloads() {
  try {
    const files = await fs.readdir(RAW_PAYLOADS_DIR)
    if (files.length > 100) {
      const sorted = files.sort()
      const toDelete = sorted.slice(0, files.length - 100)
      for (const f of toDelete) {
        await fs.unlink(path.join(RAW_PAYLOADS_DIR, f)).catch(() => {})
      }
    }
  } catch { /* ignore */ }
}
