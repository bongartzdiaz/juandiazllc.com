...
- AR — i18n on the philanthropy-core list pages: grants + volunteers. New `grants.*` and `volunteers.*` namespaces in `messages/{en,nl}.json` covering Topbar (already wired) + add/edit modal title + every form-validation toast (Title required, Funder required, Invalid amount, Save failed, Network error, etc.) + delete confirm + saved/added/updated/removed/deleted success toasts. Also added per-page `kpis.*`, `filters.*`, `fields.*`, `placeholders.*` sub-trees so future passes can wire the KPI cards + status filter dropdowns + form fields without re-extending the message files. ~30 keys per locale per page = 120 entries total.
- AS — i18n on the RE-specific list pages: showings + open-houses. New `showings.*` and `openHouses.*` namespaces with the same `{toasts,confirms}` shape — every form-validation toast (Select a property, Pick a date, End time must be after start), Save / Network error fallbacks, scheduled / updated / cancelled / deleted / marked success toasts (with `{status}` interpolation on Showings.marked), and the Cancel-this-open-house confirm. ~13 keys per locale per page = 52 entries.
- AT — org-bound industry (RE / hospitality only see their vertical). `Organization.industry` already existed as a column; this bundle wires it through to the UI. `useMySections()` extended to read `organization.industry` from `/api/me`. New `OrgIndustrySync` client component (mounted in `ProtectedShell`) calls `setOrgIndustry()` on the `IndustryProvider`, which in turn pins the active industry to the org's value for non-admins (admins keep the switcher so they can preview other verticals). Sidebar industry-switcher button hidden when `orgLocked`. Demo orgs with `industry === 'general'` keep the switcher free. No schema/migration change — just plumbing existing data into the navigation gating.
- AU — i18n on commissions / e-signatures / referrals page-level toasts + confirms. New namespaces with form-validation toasts (Select an agent, Enter a positive gross amount, Split must be 0-100; Select a transaction, Document name required, Signer email required), Save / Network error fallbacks, added/created/marked/deleted/updateFailed/deleteFailed toasts with `{status}` interpolation on `marked`, and per-page delete confirms. Dialer / inbox / lead-scores had no hardcoded toasts to wire. ~30 keys per locale across the three pages.
- AV — wire the seeded `kpis.*` / `filters.*` / `fields.*` namespaces into the grants + volunteers pages. Grants: 4 KPI cards (Total Grants, Total Funding, Active, Pending) + status-filter dropdown (10 statuses, looked up dynamically via `t(\`filters.${s}\`)`) + "New Grant" / "Edit Grant" modal title. Volunteers: 4 KPI cards (Total, Active, Total Hours, Inactive) + 3-option status filter + form fields (Name, Email, Phone, Status with onboarding/active/inactive options) + "Add Volunteer" / "Edit Volunteer" modal title + Activity Logs detail tile. Extended `grants.filters.*` to cover all 10 actual statuses (was 6 — missed prospect/under_review/approved/reporting/closed). Added `volunteers.fields.activityLogs`.
- AW — added German + Spanish to the dashboard. New `apps/philly-standalone/messages/{de,es}.json` files mirror the en.json structure (~530 lines each, every namespace covered: nav / dashboard / projects / contacts / impact / kanban / deals / properties / automations / grants / commissions / eSignatures / referrals / showings / openHouses / volunteers / common / keyboard / bulk / views / columns / filter / quickview / gdpr). ICU plural rules used for German "{# Kontakt} / {# Kontakte}" and Spanish "{# contacto} / {# contactos}". `i18n/request.ts` now accepts `de` and `es` cookie values; `useLocale.ts` extends `Locale` to all four and ships `LOCALE_LABELS` for proper switcher chrome; `toggle()` cycles EN→NL→DE→ES→EN. Topbar's existing `locale.toUpperCase()` button needs no change — it shows "DE" / "ES" automatically. Dashboard now has full locale parity with the marketing site.
- AY — lawyer-review prep on the seven legal docs. New `docs/legal/LEGAL-REVIEW-CHECKLIST.md` summarises what counsel needs to do per file (placeholder list, reviewer focus areas, sign-off criteria). Every operator-supplied field in the seven docs (`DPA`, `PRIVACY-NOTICE`, `RECORDS-OF-PROCESSING`, `SUB-PROCESSORS`, `DPIA-AI-ATTRIBUTES`, `BREACH-RESPONSE`, `COOKIE-POLICY`) was converted from the inconsistent `\<NAME>` / `\<DATE>` / `_\<…\>_` syntax to a uniform grep-friendly `[TO FILL: description]` marker. Lawyers (and operators) can find every unfinished spot via `grep -rn "\[TO FILL:" apps/philly-standalone/docs/legal/`. 30 markers across 8 files. Each marker carries an inline hint about what the substitution should look like (e.g. "[TO FILL: privacy@yourdomain.com]" instead of "[TO FILL: email]").
- AX — hospitality vertical depth. Built two new pages on top of existing schema models (Reservation + HousekeepingTask were already in `prisma/schema.prisma` from an earlier branch but had no UI route).
  - `app/reservations/page.tsx` (NEW, ~330 lines) — list with KPIs (Total / Upcoming / Checked In / Revenue), status filter, column picker, detail modal with check-in/check-out/cancel quick-actions, add-reservation modal with room picker + date validation. POST/PATCH already existed at `app/api/reservations/`.
  - `app/housekeeping/page.tsx` (NEW, ~210 lines) — kanban-board layout (Pending / In progress / Completed columns) with native HTML5 drag-drop between columns, KPIs (Total / Pending / Urgent / Completed), priority + type icons + colour, add-task modal. New `app/api/housekeeping/route.ts` (GET/POST) + `app/api/housekeeping/[id]/route.ts` (PATCH/DELETE) wire the schema model up. Tenancy enforced through the Room → org relation since `HousekeepingTask` doesn't carry its own `organizationId`. `lib/philly/audit.ts:AuditEntity` extended with `'housekeepingTask'`.
  - Sidebar `NAV_INDUSTRY` for hospitality grew from 1 item (Rooms) to 3 (Rooms / Reservations / Housekeeping). HOS-bound orgs (Bundle AT) now have a real product surface.
  - Full i18n: `reservations.*` and `housekeeping.*` namespaces added to all four message files (en/nl/de/es) — KPIs, filters, fields, statuses (kanban columns), types (cleaning/inspection/maintenance/turnover), priorities (low/medium/high/urgent), toasts, confirms. ~70 keys per locale across both pages.
  - Side fix: `nav.{rooms,reservations,housekeeping}` keys added to all four locales since the Sidebar was previously falling back to raw keys for hospitality nav items.
...
- Advanced features remaining after Bundles AA–AN:
  ...
  - Dashboard i18n sweep — DONE for shared components (AP) +
    toasts/confirms on the four major list pages (AQ) + grants
    and volunteers (AR). Remaining English-only surfaces: RE/
    realestate-specific pages (showings/open-houses/dialer/
    commissions) and the inbox master/detail layout. Most have
    a couple of toast strings + form labels that follow the same
    `<entity>.{toasts,confirms,fields,kpis,filters}` shape.
