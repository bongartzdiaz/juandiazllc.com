...
| **`CAL_WEBHOOK_SECRET`** in Vercel-productie (stap 5) | boeking → rij → Telegram; de hoofd-CTA meet nu niets | **gezet door Juan op 2026-09-21 ná de build van 13:31 UTC** (`dpl_M57r24Xy…`, #394). Gemeten 13:34 en 13:35: nog 503, en dat is verwacht — Vercel bakt env-vars in bij de build. Build `dpl_4gsmznm5…` (#395, 13:38) zei zelf `[cal] CAL_WEBHOOK_SECRET niet gezet`: **de variabele stond op Preview, niet Production**. Op Production gezet, build `dpl_993vKG9W…` (#396, 13:50) zei wéér `niet gezet`: **de waarde was leeg**. Waarde gezet ~14:0x; deze PR is de derde build; meet daarna |
