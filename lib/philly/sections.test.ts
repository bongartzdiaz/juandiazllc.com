...
describe('sectionForPath', () => {
  ...
  it('matches the canonical landing path exactly', () => {
    expect(sectionForPath('/philly/deals')?.slug).toBe('deals')
    expect(sectionForPath('/philly/contacts')?.slug).toBe('contacts')
  })

  it('inherits parent section for nested routes', () => {
    expect(sectionForPath('/philly/deals/123/edit')?.slug).toBe('deals')
    expect(sectionForPath('/philly/projects/abc')?.slug).toBe('projects')
  })

  it('picks the longest matching prefix so /philly/settings/webhooks stays in settings', () => {
    // `/philly` (dashboard) is also a prefix of `/philly/settings/*`,
    // but /philly/settings is longer so it wins.
    const match = sectionForPath('/philly/settings/webhooks')
    expect(match?.slug).toBe('settings')
  })

  it('maps /philly root exactly to the dashboard section', () => {
    expect(sectionForPath('/philly')?.slug).toBe('dashboard')
  })

  it('does not match sibling prefixes that only share a leading segment', () => {
    // `/philly/open-houses` must not match `/philly` when a more
    // specific section exists. Also verifies `startsWith` uses a slash
    // boundary — `/philly/openhouseX` should not collide with `open-houses`.
    expect(sectionForPath('/philly/open-houses')?.slug).toBe('openHouses')
    expect(sectionForPath('/philly/open-houses/feb-3')?.slug).toBe('openHouses')
  })
})
