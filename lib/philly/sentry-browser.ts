...
function beforeSend(event: BrowserEvent): BrowserEvent | null {
  // Skip health-check and analytics noise.
...
export async function initBrowserSentry(): Promise<void> {
  ...
  try {
    ...
    Sentry.init({
      ...
      // Trace sampling — keep low in prod (defaults to 0). Set
      // NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE in Vercel to lift.
      tracesSampleRate: Number(process.env.NEXT_PUBLIC_SENTRY_TRACES_SAMPLE_RATE ?? '0'),
      // Session Replay — sample 0% of normal sessions, 100% of
      // error sessions. Replay is the highest-leverage frontend
      // diagnostic; the cost shape is conservative (we only pay
      ...
      integrations: [
        Sentry.replayIntegration({
          ...
          maskAllInputs: true,
          blockAllMedia: true,
          // Don't record requests to our own auth / token endpoints —
          // even masked, network-payload bodies can leak shape.
      ...
      beforeBreadcrumb(crumb) {
        // Drop noisy console crumbs; they leak too easily.
